...
const getElectionDataFromStorageLayer = (fields, callback) => {
    ...
    let url;
    if (platform === STORAGE_LAYERS.Arweave)
        url = `${ARWEAVE_BASE_URI}/${id}`;
    else if (platform === STORAGE_LAYERS.Filecoin)
        url = `https://${id}.ipfs.w3s.link`;
    else if (platform === STORAGE_LAYERS.Pinata)
        url = `${IPFS_BASE_URI}/${id}`;
    if (!url)
        return callback("invalid_platform");
    fetch(url)
        .then((res) => res.json())
...
//# sourceMappingURL=getElectionDataByElectionId.js.map