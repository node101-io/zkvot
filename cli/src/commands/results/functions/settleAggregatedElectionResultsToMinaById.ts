  ...
  fs.access(minaPrivateKeyBase58FolderPath, fs.constants.F_OK, (err) => {
    if (err && err.code === 'ENOENT')
      return fs.mkdir(minaPrivateKeyBase58FolderPath, (err) => {
        if (err)
          return callback(err as Error | string | null);
...
const checkIfMinaPrivateKeyBase58ExistsInWalletConfig = (
  ...
  fs.readFile(minaPrivateKeyBase58FilePath, 'utf8', (err, data) => {
    ...
    try {
      ...
      if (!privKeyBase58Json.priv_key_base58)
        return callback(null, false);
...
const settleAggregatedElectionResultToMinaById = async (
  ...
  const zkAppPrivateKey = data.privateKey;
  const zkAppPublicKey = data.publicKey;
  const zkApp = new ElectionContract(zkAppPublicKey);
  ...
  try {
    await Mina.transaction(data.publicKey, async () => {
      ...
      zkApp.settleVotes(data.aggregatedElectionResult.result, data.publicKey);
    }).then(async (txn) => {
      await txn.prove();
...
readWalletConfigAndGetMinaPrivateKey(minaPrivateKeyBase58FilePath, (err, minaPrivateKey) => {
  ...
  if (!minaPrivateKey)
    return logger.log('error', 'priv_key_not_found');
  ...
  getMinaPublicKeyByPrivateKey(minaPrivateKey, (err, minaPublicKey) => {
    ...
    if (!minaPublicKey)
      return logger.log('error', 'pub_key_not_found');
    ...
    settleAggregatedElectionResultToMinaById({
      ...
      aggregatedElectionResult: {
        electionId: PublicKey.fromBase58('B62qkdfkUfGoz6bXrupHhnyE5e6NQJLDKXvWQaWVNDFKV92TUDw1zg6'),
...
const settleCheck = (
  ...
  createMinaWalletConfigFolderIfNotExists(minaPrivateKeyBase58FolderPath, (err) => {
    ...
    createMinaWalletConfigFileIfNotExists(minaPrivateKeyBase58FilePath, (err) => {
      ...
      checkIfMinaPrivateKeyBase58ExistsInWalletConfig(minaPrivateKeyBase58FilePath, (err, exists) => {
        if (err)
          return callback(err);

        if (!exists) {
          logger.log('warn', `Please add your Mina private key to the wallet config: ${ADD_MINA_PRIVATE_KEY_TO_WALLET_CONFIG_COMMAND}`);
        ...
        readWalletConfigAndGetMinaPrivateKey(minaPrivateKeyBase58FilePath, (err, minaPrivateKey) => {
          ...
          getMinaPublicKeyByPrivateKey(minaPrivateKey, (err, minaPublicKey) => {
            ...
            if (!minaPublicKey)
              return callback('pub_key_not_found');
