...
const installCelestiaLightNodeIfRequired = (daLayerInfo, callback) => {
    ...
    const celestiaInfo = daLayerInfo.find(
        (item) => typeof item === "object" && item.name === "celestia"
    );

    if (!celestiaInfo) return callback(null);

    if (!celestiaInfo.start_block_hash || typeof celestiaInfo.start_block_hash != "string")
        return callback("bad_request");

    if (
        !celestiaInfo.start_block_height ||
...
export default (election, callback) => {
    if (!election || typeof election != "object") return callback("bad_request");
