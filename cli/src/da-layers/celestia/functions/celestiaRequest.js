...
export default (url, options, callback) => {
    if (!url || !validator.isURL(url.toString())) return callback("bad_request");

    if (!options || typeof options != "object") return callback("bad_request");
    ...
    if (!options.params || !Array.isArray(options.params)) return callback("bad_request");

    db.get("celestia_auth_key", (err, value) => {
        if (err && err.code == "LEVEL_NOT_FOUND") return callback("key_not_found");
        ...
        fetch(url, {
            ...
            .catch((err) => {
                if (DATA_NOT_FOUND_ERROR_MESSAGE_REGEX.test(err)) return callback("data_not_found");
