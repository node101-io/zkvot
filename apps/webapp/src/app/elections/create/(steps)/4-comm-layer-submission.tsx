...
export default ({
  ...
  const { subwalletAccount, isSubmitting, createAppId, connectSubwallet } =
  ...
  useEffect(() => {
    ...
    if (communicationLayer && communicationLayer.name === "Celestia")
      setBlockHash(
        (communicationLayer as types.CelestiaDaLayerInfo).start_block_hash
      );
  }, [initialData]);
  ...
  const handleNext = () => {
    ...
    if (
      communicationLayer.name == "Celestia" &&
      blockHeight < 0 &&
      !blockHash?.trim().length
    )
      return;
    ...
    <div className="flex flex-col justify-between items-center space-y-6 min-h-[calc(100vh-160px)] max-h-[calc(100vh-162px)] overflow-y-auto p-4">
      <div className="w-full">
        ...
        <input
          ...
          value={blockHeight}
          onInput={(event) =>
            setBlockHeight(Number((event.target as HTMLInputElement).value))
          }
        ...
        {initialData.communication_layers[0].name === "Avail" ? (
          <>
            ...
            <input
              type="text"
              value={
                (initialData.communication_layers[0] as types.AvailDaLayerInfo)
                  .app_id
              }
              readOnly
              className="w-full max-w-[620px] h-12 p-2 focus:outline-none bg-[#1E1E1E] text-[#B7B7B7] rounded-[50px] my-4"
          ...
          <>
            ...
            <input
              ...
              value={blockHash}
              onInput={(event) =>
                setBlockHash((event.target as HTMLInputElement).value)
              }
              className="w-full max-w-[620px] h-12 p-2 focus:outline-none bg-[#1E1E1E] text-[#B7B7B7] rounded-[50px] my-4"
            />
          </>
      ...
      <div className="w-full flex justify-between pt-4">
        ...
        <Button
          ...
          disabled={
            ...
            (initialData.communication_layers[0].name === "Avail" &&
              blockHeight < 0) ||
            (initialData.communication_layers[0].name === "Celestia" &&
              blockHeight < 0 &&
              !blockHash?.trim().length)
          }
          className={`${
