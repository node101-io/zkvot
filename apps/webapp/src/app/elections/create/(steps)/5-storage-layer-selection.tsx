...
export default ({
  ...
  const [selectedStorageLayer, setSelectedStorageLayer] = useState<
    types.StorageLayerPlatformCodes | undefined
  >();
  ...
  return (
    <div className="flex flex-col justify-between items-center space-y-6 min-h-[calc(100vh-160px)] max-h-[calc(100vh-162px)] overflow-y-auto p-4">
      <div className="w-full">
        <div className="mb-4">
          <h3 className="text-white text-xl">Choose a Storage Layer</h3>
        </div>
        <div className="w-full">
          <div className={`grid grid-cols-1 sm:grid-cols-2 gap-4 w-full`}>
            {(
              Object.keys(
                StorageLayerDetails
              ) as types.StorageLayerPlatformNames[]
            ).map((layer: types.StorageLayerPlatformNames, index) => (
              <div
                key={index}
                ...
                onClick={() =>
                  setSelectedStorageLayer(StorageLayerDetails[layer].code)
                }
              >
                <div className="w-[160px] h-[160px] flex-shrink-0 rounded-lg overflow-hidden mr-4 flex items-center justify-center">
                ...
                <div className="flex flex-col h-full justify-between">
                  <h3 className="text-white text-[24px] mb-2">
                    {String(layer).charAt(0).toUpperCase() +
                      String(layer).slice(1)}
                  </h3>
                  <p className="text-[16px] mb-2">
                    {StorageLayerDetails[layer].description}
                  </p>
