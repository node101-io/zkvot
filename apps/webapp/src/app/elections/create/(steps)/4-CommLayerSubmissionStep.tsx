...
import CreateAppId from '@/app/elections/create/(partials)/CreateAppId.jsx';
...
export default ({
  ...
  onUpdateElectionData: (data: any) => void;
}) => {
  ...
  useEffect(() => {
    ...
    if (communicationLayer) {
      ...
      } else if (communicationLayer.name === 'celestia') {
  ...
  const fetchCelestiaBlockDataInStepFour = async () => {
    try {
      const data = await fetchCelestiaBlockInfoFromBackend();
      ...
      onUpdateElectionData((prevData: any) => {
  ...
  const fetchAvailBlockHeightInStepFour = async () => {
    try {
      const height = await fetchAvailBlockHeightFromBackend();
  ...
  return (
    <div className='flex flex-col justify-between items-center space-y-6 h-[calc(100vh-215px)] overflow-y-auto p-4'>
      ...
      {electionData.communication_layers[0].name === 'celestia' && (
        <>
