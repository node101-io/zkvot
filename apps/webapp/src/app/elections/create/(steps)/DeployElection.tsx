...
import { calculateMinaBlockHeightFromTimestampViaBackend } from "@/utils/backend";
import {
...
export default ({
  ...
  const handleSubmit = async () => {
    ...
    if (isSettingUp) {
      showToast(
        "Loading ZK, please wait a few more minutes and try again.",
        "error"
      );
      return;
    }
    ...
    return data.election.communication_layers.map((layer, index) => (
      ...
      >
        <div className="w-16 h-16 flex-shrink-0 rounded-md mr-4 flex items-center justify-center bg-gray-500">
          {CommunicationLayerDetails[
            layer.name as keyof typeof CommunicationLayerDetails
  ...
  const renderStorageLayer = () => {
    const layer =
      StorageLayerDetails[
        data.storage_layer_platform as keyof typeof StorageLayerDetails
      ];
    ...
    return (
      <div className="flex items-center bg-[#222222] p-4 rounded-2xl mb-4">
        ...
        <div className="flex flex-col">
          <h3 className="text-white text-lg mb-1 capitalize">
            {data.storage_layer_platform}
          </h3>
          <span className="text-sm text-gray-400">
              ...
              <div className="text-[#B7B7B7] text-sm mb-2 flex flex-row items-center ">
                <span className="flex flex-row justify-center items-center ">
                  ...
                  <span className="text-sm text-[#B7B7B7]">
                    Start Date: {formatDate(data.election.start_date)}, End
                    Date: {formatDate(data.election.end_date)}
                  </span>
                </span>
              </div>
            </div>
