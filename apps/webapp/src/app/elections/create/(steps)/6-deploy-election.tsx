...
import { checkIfAccountExists } from '@/utils/o1js.js';
...
export default ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      result = await zkProgramWorkerClientInstance.deployElection(
        address,
        data.election.start_slot,
        data.election.end_slot,
  ...
  const renderOptionalFields = (voter: types.Voter) => {
    const optionalFields = data.election.voters_list?.length > 0 ?
      Object.keys(data.election.voters_list[0]).filter((key) => key !== 'public_key') : [];
            ...
            <div className='md:ml-6 flex-1'>
              <div className='flex items-center mb-2 text-gray-400 text-sm space-x-1'>
                ...
                <div className='flex space-x-2'>
                  <div>
                    <span className='font-medium '>Starting at Mina slot: </span>
                    {data.election.start_slot}
                  </div>
                  <div>
                    <span className='font-medium '>Ending at Mina slot: </span>
                    {data.election.end_slot}
                  </div>
                </div>
              </div>
