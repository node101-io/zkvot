...
export default function ResultsPage({ params }: { params: { id: string } }) {
  ...
  const verifySoftFinalityProof = async (softFinalityResult: number[], softFinalityProof: string) => {
    try {
      const verificationKey = zkProgramWorkerClientInstance ? await zkProgramWorkerClientInstance.getVerificationKey() : Aggregation.verificationKey;
  ...
  useEffect(() => {
    ...
    fetchElectionData()
      .then((data: types.ElectionBackendData) => {
        ...
        setHardFinalityResult(data.result.slice(0, data.options.length).map(each => {
    ...
    fetchElectionResultByContractIdFromBackend(params.id)
      .then(data => {
        ...
        verifySoftFinalityProof(data.result.map(result => Number(result.voteCount)), softFinalityProof);
      })
      .catch(_ => {
    ...
    <div className='w-full flex flex-col items-start space-y-2'>
      <div className='flex items-center justify-start w-full'>
        ...
        <span className='text-white text-[14px] pl-2'>
          %{isNaN(result.percentage) ? 0 : result.percentage} ({Number(result.voteCount).toLocaleString()})
        </span>
      </div>
      <div className='w-full bg-[#434446] rounded-full overflow-hidden h-[30px]'>
        <motion.div
          ...
          initial={{ width: '0%' }}
          animate={{ width: `${isNaN(result.percentage) ? 0 : result.percentage}%` }}
              ...
              <div className='p-6 w-full h-full flex flex-col justify-between'>
                <div className='flex flex-row w-full justify-between items-center mb-4'>
                  ...
                  <span className='flex flex-row justify-center items-center text-sm text-[#B7B7B7]'>
                    <Clock />
                    <span className='ml-2'>
                      <DateFormatter date={electionData?.start_date || new Date()} />
                    </span>
                  </span>
                </div>
                <div className='flex-grow'>
                  <h2 className='text-3xl font-semibold text-white mb-3'>{electionData?.question}</h2>
                  <p className='text-[#E0E0E0] italic mb-6'>{electionData?.description}</p>
