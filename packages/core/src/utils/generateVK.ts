...
await fs.writeFile(
  path.join(
    import.meta.dirname,
    '../../../core/src/verification-keys/VoteVK.ts'
  ),
  `export const verificationKey = '${JSON.stringify(voteVerificationKey)}';`
);
await fs.writeFile(
  path.join(
    import.meta.dirname,
    '../../../core/src/verification-keys/AggregationVK.ts'
  ),
  `export const verificationKey = '${JSON.stringify(
    aggregationVerificationKey
  )}';`
);
