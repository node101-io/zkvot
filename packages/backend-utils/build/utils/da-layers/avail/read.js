...
export default async (height, is_devnet, callback) => {
    let submission_data_list = [];
    const defaultAppId = is_devnet ? config.devnet.appID : config.mainnet.appID;
    try {
        if (!defaultAppId)
        ...
        for (let i = 0; i < submitDataTransactions.length; i++) {
            ...
            try {
                const submittedDataAppId = Number(extrinsic.__internal__raw.signature.appId);
